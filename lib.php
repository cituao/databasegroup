...
/**
 * Databasegroup enrolment plugin.
 *
 * This plugin synchronises enrolment and roles with external databasegroup table.
 *
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_databasegroup_plugin extends enrol_plugin {
    /**
    ...
    public function can_delete_instance($instance) {
        ...
        if (!has_capability('enrol/databasegroup:config', $context)) {
            return false;
        }
        if (!enrol_is_enabled('databasegroup')) {
            return true;
        }
    ...
    public function can_hide_show_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/databasegroup:config', $context);
    }
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol_user($instance, $ue) && has_capability('enrol/databasegroup:unenrol', $context)) {
    ...
    /**
     * Forces synchronisation of user enrolments with external databasegroup,
    ...
    public function sync_user_enrolments($user) {
        ...
        $allroles = get_all_roles(); //accesslib.php
        ...
        // Read remote enrols and create instances. Read records user remote
        ...
        foreach ($roleassigns as $courseid => $roles) {
            //curso no existe en las instancias de bdmoodle saltar
        ...
        $sql = "SELECT e.*, c.visible AS cvisible, ue.status AS ustatus
             ...
             LEFT JOIN {user_enrolments} ue ON ue.enrolid = e.id
                 WHERE ra.userid = :userid AND e.enrol = 'databasegroup'";
        ...
        foreach ($rs as $instance) {
            ...
            //si esta vacía es true pasa a false hay que desmatricular 
            //si esta lleno es false pasa a true hay que mantener la matricula
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                    ...
                    role_unassign_all(array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_databasegroup', 'itemid'=>$instance->id));
                }
            }
        }
        $rs->close();
    ...
    /**
     * Forces synchronisation of all enrolments with external databasegroup.
     *
    ...
    public function sync_enrolments(progress_trace $trace, $onecourse = null) {
        ...
        if (!$extdb = $this->db_init()) {
            $trace->output('Error while communicating with external enrolment databasegroup');
            $trace->finished();
            return 1;
        }
...
/*SELECT c.id, c.visible, c.shortname AS mapping, e.id AS enrolid
FROM course c
LEFT JOIN enrol e ON (e.courseid = c.id AND e.enrol = 'databasegroup')
WHERE c.id = 463 */
        ...
        if ($onecourse) {
            ...
            //if not there are record in enrol we create instances j
            ...
            //externalcourses tiene los cursos que estan en la bd remota 
            ...
            // First find all existing courses with enrol instance type databasegroup.
            $existing = array();
            /* SELECT c.id, c.visible, c.shortname AS mapping, e.id AS enrolid, c.shortname
FROM mdl_course c
JOIN mdl_enrol e ON (e.courseid = c.id AND e.enrol = 'databasegroup')*/
            
            $sql = "SELECT c.id, c.visible, c.$localcoursefield AS mapping, e.id AS enrolid, c.shortname
                      FROM {course} c
                      JOIN {enrol} e ON (e.courseid = c.id AND e.enrol = 'databasegroup')";
            $rs = $DB->get_recordset_sql($sql); // Watch out for idnumber duplicates.
            foreach ($rs as $course) {
                if (empty($course->mapping)) {
                    //this course not have mapping refuse
                    continue;
                }
                //add course in existing 
            ...
            //$externalcourses tiene los cursos que no tiene instancia de la base de datos
            ...
            /*SELECT c.id, c.visible, c.shortname AS mapping, c.shortname
                FROM mdl_course c
                LEFT JOIN mdl_enrol e ON (e.courseid = c.id AND e.enrol = 'databasegroup')
                WHERE e.id IS NULL AND c.shortname <> '' */
            
            //buscamos todos los cursos que no tiene instancias
            ...
            foreach ($rs as $course) {
                ...
                //si el curso esta y no es null es TRUE lo invertimos para que no entre
                //si el curso sin instancia esta en la bd externa invertimos para que lo agregue
                //de lo contrario sino esta lo ignoramos es un curso que no esta la bdexterna
                if (!isset($externalcourses[$course->mapping])) {
                    // Course not synced or duplicate.
                    continue;
                }
                //agregamos la instancia del curso en enrol
                $course->enrolid = $this->add_instance($course);
                //agregamos el curso en existing
                $existing[$course->mapping] = $course;
                unset($externalcourses[$course->mapping]);
            }
            $rs->close();
        ...
        foreach ($existing as $course) {
            ...
            /* SELECT u.username AS mapping, u.id AS userid, ue.status, ra.roleid
                FROM mdl_user u
                JOIN mdl_role_assignments ra ON (ra.userid = u.id AND ra.component = 'enrol_databasegroup' AND ra.itemid = 1452)
                LEFT JOIN mdl_user_enrolments ue ON (ue.userid = u.id AND ue.enrolid = ra.itemid)
                WHERE u.deleted = 0 */
            
            //obtener una lista de usuarios del curso con sus roles y con instancias o no 
            $sql = "SELECT u.$localuserfield AS mapping, u.id AS userid, ue.status, ra.roleid
                      FROM {user} u
                      JOIN {role_assignments} ra ON (ra.userid = u.id AND ra.component = 'enrol_databasegroup' AND ra.itemid = :enrolid)
                 LEFT JOIN {user_enrolments} ue ON (ue.userid = u.id AND ue.enrolid = ra.itemid)
                     WHERE u.deleted = 0";
            //itemid es el id de la instancia en la tabla enrol
            ...
            //usermapping tiene todos los usuarios del curso
            //currentstatus tiene los estudiantes matriculados actualmente en el curso
            //currentenrols tiene los estudiantes matriculados actualmente en el curso
            ...
            if ($rs = $extdb->Execute($sql)) {
                if (!$rs->EOF) {
                    ...
                    //iterar por todos los usuarios de la base de datos externa
                    while ($fields = $rs->FetchRow()) {
                        //leemos cada usuario (base de datos externa) y validamos
                        $fields = array_change_key_case($fields, CASE_LOWER);
                        ...
                        if (!isset($usermapping[$mapping])) {
                            //este usuario es nuevo no esta en el curso en proceso
                            $usersearch[$localuserfield] = $mapping;
                            if (!$user = $DB->get_record('user', $usersearch, 'id', IGNORE_MULTIPLE)) {
                                //usuario externo no esta en siga
                                $trace->output("error: skipping unknown user $localuserfield '$mapping' in course '$course->mapping'", 1);
                                continue;
                            }
                            //es un nuevo usuario lo agregamos a la lista de usuarios del curso 
                            //esto con el fin de evitar un reproceso
                        ...
                        if (empty($fields[$otheruserfieldlower])) {
                            //llenamos el array usuario y role correspondiente
                ...
                $trace->output("error: skipping course '$course->mapping' - could not match with external databasegroup", 1);
                continue;
            }
            unset($usermapping);
            
            //$requestenrols tiene los usuarios a matricular
            //
            // Enrol all users and sync roles.
            foreach ($requestedenrols as $userid => $userroles) {
                foreach ($userroles as $roleid) {
                    if (empty($currentenrols[$userid])) {
                        //sino esta en la lista de los matriculados lo matriculamos 
                ...
                //Si en siga esta suspendido reestablecemos su estado
                // Reenable enrolment when previously disable enrolment refreshed.
            ...
            //requestedroles roles de la base de datos externa
            
            foreach ($requestedroles as $userid => $userroles) {
                // Assign extra roles.
                foreach ($userroles as $roleid) {
                    if (empty($currentroles[$userid][$roleid])) {
                        role_assign($roleid, $userid, $context->id, 'enrol_databasegroup', $instance->id);
                        $currentroles[$userid][$roleid] = $roleid;
                ...
                foreach ($currentroles[$userid] as $cr) {
                    //si hay un rol en siga que no se encuentra en la base de datos externo lo quitamos
                    if (empty($userroles[$cr])) {
                        role_unassign($cr, $userid, $context->id, 'enrol_databasegroup', $instance->id);
            ...
            //currentroles tiene roles que no estan en la base de datos externa
            foreach ($currentroles as $userid => $userroles) {
                // These are roles that exist only in Moodle, not the external databasegroup
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                foreach ($currentstatus as $userid => $status) {
                    ...
                    if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                        ...
                        role_unassign_all(array('contextid'=>$context->id, 'userid'=>$userid, 'component'=>'enrol_databasegroup', 'itemid'=>$instance->id));
    ...
    /**
     * Performs a full sync with external databasegroup.
     *
    ...
    /**
     * Tries to make connection to the external databasegroup.
     *
    ...
    protected function db_init() {
        ...
        // Connect to the external databasegroup (forcing new connection).
    ...
    public function test_settings() {
        ...
        if (!$adodb or !$adodb->IsConnected()) {
            ...
            echo $OUTPUT->notification('Cannot connect the databasegroup.', 'notifyproblem');
            return;
        }
